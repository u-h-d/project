...
(define-for-syntax l3
  '(hello thing done))
                   

(define l2
  '(test name length field))
...
; works, but how to pass in an identifier that we can operate on associated value
; can we use nested 'with-syntax' -> use match or whatever to get the values (need to be 'define-for-syntax', then make them pattern vars with 'with-syntax'
; we have to have a way to evaluate the id's...
; THEN use those new, bound pattern vars in a template to get our desired code...
...
(define v
  '(test f1 f2 f3))
(define-for-syntax (helper l)
  (car (syntax->datum l)))

; might just have to use eval...
(define-syntax (make-st stx)
  (syntax-parse stx
    ((_ lst)
     (displayln #'lst)
     (define l #'lst)
     ...
     (with-syntax
         ((new-l l)
          (nl (eval #'lst)))
       ;(define h (helper #'new-l))
       (displayln #'nl)
       #''nl))))

; ** MAYBE have macro pull car as 'name', cdr as 'fields', then use 'match' to match field ..., then return syntax that is (field ...)
; to be inserted into #'(define struct name (fields ...)) and maybe have that inserted into some data structure


; !!! -> syntax-local-value/immediate page where the use #`'stx format...

#| !! syntax-local-eval not working...
(define-syntax (mact stx)
  (syntax-parse stx
    ((_ expr)
     (begin
       (displayln (syntax-local-eval #'expr))
       #'expr))))
|#
